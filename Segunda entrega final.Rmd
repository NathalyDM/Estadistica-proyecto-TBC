...
Vs=(seq(from=mean(tasademortalidadprom[1]),to=mean(tasademortalidadprom[7]),by=(mean(tasademortalidadprom[1])-mean(tasademortalidadprome[7]))/6))
...
for(i in 2012:2018){
  z<-z+1
  tasademortalidad[z]<- sum(datos$Tasa.de.mortalidad[datos$Año==i])
  años[z]<-i
}
cases<-tasademortalidad
names(cases)<-años
