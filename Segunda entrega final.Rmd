...
<p style="text-align: justify;">- La mayoría de datos serán recopilados directamente del Portal de la DPCTB - Dirección de Prevención y Control de Tuberculosis, bajo la direccion del  <a href=”http://www.tuberculosis.minsa.gob.pe/DashboardDPCTB/PerfilTB.aspx”>MINSA.</a>. Así mismo, las variables a la que corresponden estos datos serían las tasas de mortalidad, las tasas de inicidencia,  morbilidad, la tasa de éxito y el número de casos de TBC y de casos nuevos. Por último, las referencias de esta fuente se encuentran en la bibliografía del presente documento.</p>

<p style="text-align: justify;">- Nuestra segunda fuente de datos está conformada por el Ministerio de Economía y Finanzas, específicamente por la sección de <a href=”https://www.mef.gob.pe/es/presupuesto-del-sector-publico/proyecto-de-presupuesto”>Presupuesto para el sector Público y los Proyectos de Presupuestos Anuales (Anexo 8)</a>. Y como la fuente anterior, su correcta cita bibliográfica se encuentra en la parte inferior del documento.</p>

<p style="text-align: justify;">- Por último, nuestra tercera fuente de datos la conforma el Instituto de Estadística e Informática., específicamente la sección de <a href=”https://www.inei.gob.pe/estadisticas/indice-tematico/a1-condiciones-fisicas-10234/>Condiciones Físicas</a>, la cita correspondiente está en la bibliografía del documento.</p>
...
for (i in 2012:2018){
       x<-x+1
       morbilidad_porr_anho[x]<-mean(datos$Numero.de.casos.Nuevos[datos$Año==i])
       anhos[x]<-i
}
...
<p style="text-align: justify;">Para determinar el presupuesto mas bajo y alto en el periodo a analizar se utilizarán la media con respecto a la distribución en los años 2012 a 2018 </p> 
...
datos5<-data.frame(departamentos1,Morbilidad2,Presupuesto.orientado2)
...
<p style="text-align: justify;">En el Gráfico N°3 se puede observar la tasa promedio de mortalidad por TBC en el Perú. En 2012 comenzó con una tasa promedio de **`r format (tasademortalidadprom[1])`**, al año siguiente la tasa bajo hasta **`r format (tasademortalidadprom[2]) `** la cual es la tasa más baja en nuestro estudio. En el 2014 se presentó la tasa más alta de mortalidad, la cual fue **`r format (tasademortalidadprom[3])`**. En los años siguiente se mantuvo una tasa constante de **`r format (mean(tasademortalidadprom[4], tasademortalidadprom[5], tasademortalidadprom[6], tasademortalidadprom[7]))`**.</p> 

<p style="text-align: justify;">Encontramos la variación porcentual entre el 2012 y 2018 `(tasademortalidadprom[7]-tasademortalidadprom[1])/tasademortalidadprom[1])*100` y lo dividimos entre 6 y obtenemos **`r format (round((tasademortalidadprom[7]-tasademortalidadprom[1])/tasademortalidadprom[1]*100/6,2))`%**; aproximadamente la tasa de mortalidad promedio en Perú **ha aumentado 1%** por año.</p> 
...
datos3<-data.frame(departamentos1,Numero.de.casos.nuevos2)
...
barplot(datos3$Numero.de.casos.nuevos2,main = "Gráfico N°4: Nº de casos nuevos de TBC por departamento",width=0.85 ,ylab =  "Número de casos nuevos de TBC", col=rainbow(25),ylim = c(0,8000))
...
cases<-tasademortalidadprom
names(cases)<-años
barplot(cases,main="Gráfico N°5: Tasa de mortalidad promedio desde el 2012 hasta el 2018 en el Perú"
        ,ylab ="Tasa de mortalidad promedio"
        ,xlab ="Años"
        ,ylim=c(0,4)
        ,cex.main=1
        ,cex.lab=0.8
        ,col = "#1234F8")
```
<p style="text-align: justify;">Como podemos observar a partir de los datos representados en el Gráfico N°5 de barras, la tasa de mortalidad disminuyó de manera constante durante los primeros 6 años; mientras que para el 2018 esta disminuyó con una pendiente más pronunciada; es decir, la tasa de mortalidad disminuyó de una manera más eficiente. Esto se puede dar debido a diversos factores que se analizarán más adelante, como por ejemplo, el presupuesto invertido por diversas instituciones.</p> 
...
names(casos)<-años
barplot(casos
        ,main="Gráfico N°6: N° de casos de TBC desde el 2012 hasta el 2018 en el Perú"
        ,ylab ="N* de casos de TBC (miles de personas)"
        ,xlab ="Años"
        ,cex.main=1
        ,cex.lab=0.8
...
plot(Presupuesto_millones,tasa_mortalidad 
     ,main="Gráfico N°7: Presupuesto invertido vs tasa de mortalidad por TBC"
...
<p style="text-align: justify;"> Según el Gráfico N°7 podemos observar que no existe una correlación entre el presupuesto invertido y la tasa de mortalidad a nivel nacional. El coeficiente de correlación es 6.624%, y el aceptado para poder establecer alguna correlación debe ser mayor a 95%. De este modo, no existe algún patrón o tendencia entre el presupuesto invertido y la tasa de mortalidad.</p> 
...
par(mfrow=c(1,2))
x<-7
for (i in 1:length(departamentos1)){
Presupuesto_millones_dep<-datos$Presupuesto.para.el.programa.orientado.a.disminuir.la.incidencia.de.TBC.y.VIH[datos$Departamento==departamentos1[i]]/1000000
tasa_mortalidad<-datos$Tasa.de.mortalidad[datos$Departamento==departamentos1[i]]
cor(Presupuesto_millones_dep, tasa_mortalidad)
coeficiente<-round(cor(Presupuesto_millones_dep, tasa_mortalidad),2)
if(abs(coeficiente)>=0.5){
x<-x+1
nombre_depa<-c(paste("Gráfico N°:",toString(x),"Presupuesto vs Tasa",sep = " "),paste(" de mortalidad en\t", toString(departamentos1[i]),sep = " "))
plot(Presupuesto_millones_dep 
     ...
     ,cex.main=0.75
     ,cex= 0.75)
abline(lm(tasa_mortalidad~Presupuesto_millones_dep), col="blue")
coeficiente_es<-c("r:",coeficiente)
legend((max(Presupuesto_millones_dep)+min(Presupuesto_millones_dep))/2,  40,legend=c(coeficiente_es),cex=0.8)
}
}
```
...
P<-Presupuesto.orientado2/1000000
barplot(Morbilidad2,main="Gráfico N°18: Morbilidad de TBC por departamento"
...
barplot(P,main="Gráfico N°19: Presupuesto invertido por departamento"
        ,ylab = "Presupuesto en millones de soles"
        ,width = 0.85
        ,col = "skyblue"
        ,ylim =c(0,20)
        ,cex.main=1)
axis(1,at=1:25,labels = departamentos1,las=3,cex.axis=0.7)
abline(h=4.676521,col="red",lwd=1)
q1<-quantile(P)
q2<-quantile(Morbilidad2)

```

<p style="text-align: justify;">Luego, para identificar los departamentos de menor presupuesto en el Gráfico N°19, se seleccionaran los datos que se encontraban por debajo del primer cuartil de los datos, esto se logró utilizando el código `quantile(P)` que permite hallar los cuartiles de P. De esa forma se identifica el primer cuartil y se arma una tabla con los datos que tienen un presupuesto menor al del primer cuartil que es de **4676521 soles**</p> 
```{r}
dato6<-datos5$departamentos1[which(datos5$Presupuesto.orientado2<=4676521)]
morb3<-datos5$Morbilidad2[which(datos5$Presupuesto.orientado2<=4676521)]
```
<p style="text-align: justify;">A la vez, se determinó que los departamentos con media de presupuesto inferior al primer cuartil corresponde a los departamentos de **`r format (datos5$departamentos1[which(datos5$Presupuesto.orientado2<=4676521)])`**</p> 

Ahora en el Gráfico N°20, se observan los datos de morbilidad correspondientes a estos departamentos.
```{r}
barplot(morb3,main="Gráfico N°20: Morbilidad de departamentos con bajo presupuesto"
        ,ylab = "Morbilidad"
        ,width = 0.85
        ,col = "skyblue"
        ,ylim =c(0,1000)
...
```{r}
plot(x=Numero.de.casos.nuevos,y=Presupuesto_millones
     ,xlab = "Número de casos nuevos por departamento"
     ,ylab = "Presupuesto por departamento (millones de soles)"
     ,main = "Gráfico N°21: Presupuesto invertido VS. Número de casos nuevos"
     ,cex.main=1)
plot(x = Numero.de.casos.nuevos,y = Presupuesto_millones
     ,ylim = c(0,30),xlim=c(0,1250)
     ,xlab = "Número de casos nuevos por departamento"
     ,ylab = "Presupuesto por departamento (millones de soles)"
     ,main = "Gráfico N°22: Presupuesto invertido VS. Número de casos nuevos (ampliado)"
     ,cex.main=1)
abline(lm(Presupuesto_millones~Numero.de.casos.nuevos), col="blue")
coeficiente_es<-c("r:",round(cor(Numero.de.casos.nuevos,Presupuesto_millones),2))
legend(1100, 30,legend=c(coeficiente_es),cex=0.8)
```

<p style="text-align: justify;">Como se puede observar en el Gráfico N°21 la mayoría de los datos se encuentran en una área muy pequeña, mientras que hay 6 datos atípicos en los extremos. Por lo tanto descartamos estos datos atípicos y disminuimos el rango en el eje "X" y "Y" para obtener el segundo gráfico.</p>

<p style="text-align: justify;">En el Gráfico N°22 se puede encontrar un pequeño conjunto de valores en la esquina inferior izquierda. Esta relación nos indica que hay una tendencia que a menor número de casos nuevos por departamento, menor será el presupuesto invertido para aquel departamento. Sin embargo el coeficiente de correlación en este gráfico es de un 10% lo cual nos indica que no existe una relación directa entre el número de casos nuevos y el presupuesto invertido por departamentos. Por ello se pueden observar departamentos en los que se tienen altos número de casos nuevos pero tienen menor presupuesto que comparado con otros con menor número de casos nuevos.</p>
...
Ministerio de Economía y Finanzas (2012-2018). Proyecto de Presupuesto (Nacional, Regional y Local), Anexo N°8. Recuperado 12 mayo, 2019, de https://www.mef.gob.pe/es/presupuesto-del-sector-publico/proyecto-de-presupuesto
...
Ministerio de salud. (2018). Situación epidemiológica del VIH-Sida en el Perú. Boletín mensual Diciembre 2018: Centro Nacional de Epidemiología, Prevención y Control de Enfermedades.
